...
package FS.Elite.SlidingWindowDay2;

import java.util.*;
public class P2{
    public static int calculateScore(HashMap<Integer,Integer> m, int f, int x){
        PriorityQueue<int[]> pq = new PriorityQueue<>((a,b)->Integer.compare(b[0],a[0])); 
        for(int key : m.keySet()){
            pq.add(new int[]{m.get(key)*(int)Math.pow(key,f),key});
        }
        int total = 0;
        for(int i = 0;i<x;i++){
            int cur[]=pq.poll();
            total+=m.get(cur[1])*cur[1];
        }
        return total;
    }
    public static void main(String args[]){
        ...
        scores.add(calculateScore(fmap,f,x));
        for(int i =k;i<n;i++){
            // add cur
            fmap.put(a[i],fmap.getOrDefault(a[i],0)+1);
            // remove first
            fmap.put(a[i-k],fmap.get(a[i-k])-1);
            scores.add(calculateScore(fmap,f,x));
        }
